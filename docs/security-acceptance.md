...
| Device authorization is offered only when advertised, binds the verification URI to the site origin, backs off on `slow_down`, discards any grant that arrives after the earlier of code expiry and `--timeout`, keeps a client registered for one grant out of the other, and stores nothing on denial | `test/oauth-login-contract.test.mjs` |
...
| Bounded caches/output and private credential/artifact storage | `test/cache-artifacts-contract.test.mjs`; `test/credentials-contract.test.mjs`; `test/state-contract.test.mjs` |
| Windows protected ACLs owned by the current user, granting the current user and optionally SYSTEM and `BUILTIN\Administrators`, with every rule explicit, `Allow`, full control, correctly flagged and unique; Deny, inherited rules, unexpected principals, duplicates, insufficient rights, and invalid flags rejected; applied and verified in batch through one bounded helper process, with paths passed on stdin and a failed apply accepted only against a postcondition that verifies as safe | `test/state-contract.test.mjs`; `scripts/windows-acl-acceptance.mjs` |
